...
export function useFiltersState({
  ...
  const initial: FiltersState = useMemo(() => {
    ...
    const max =
      Number.isFinite(initialPriceMax) && initialPriceMax > min
        ? initialPriceMax
        : Math.max(min + 1, 200);
  ...
  const setLocations = useCallback(
    (locations: string[]) => setFilters(s => ({ ...s, locations })),
    [],
  );
  const setCategory = useCallback(
    (category: string) => setFilters(s => ({ ...s, category })),
    [],
  );
  const setPriceRange = useCallback(
    (priceRange: [number, number]) => setFilters(s => ({ ...s, priceRange })),
    [],
  );
