...
export default function SearchPageClient({
  ...
  const itemsSource =
    providerIds && query.data?.items && query.data.items.length > 0
      ? query.data.items
      : items;
  const {
    ...
    price: priceBounds,
  } = useDerivedFilters(items);
  const { filters, replace } = useFiltersState({
    initialPriceMin: priceBounds.min,
    initialPriceMax: priceBounds.max,
  });

  const handleFiltersChange = useCallback(
    (next: typeof filters) => replace(next),
    [replace],
  );
  ...
  const filtered = useMemo(() => {
    return itemsSource.filter(it => {
      const locationMatch =
        ...
        filters.locations.includes(it.location || '');
      const categoryMatch =
  ...
  return (
    <LayoutProvider>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <SectionHeader
          title="All Tours"
          subtitle="Explore Tours, Tickets & Activities"
        />
