...
function formatFeeRatesFriendly(
  feeRates?: Array<{
    start?: number | null;
    end?: number | null;
    rate: number;
  ...
  for (const r of arr) {
    ...
    if (r?.start == null && r?.end != null) {
      ...
      rows.push(
        `${Number(r.start)}–${Number(r.end)} days before: ${rate}% fee${suffix}`,
      );
    }
  }
  return rows;
...
function computePivotHoursFromFees(
  feeRates?: Array<{
    start?: number | null;
    end?: number | null;
    rate: number;
  ...
  if (pivotDay != null && Number.isFinite(pivotDay))
    return Math.max(0, pivotDay) * 24;
...
export function CancellationPolicy({
  ...
  const workingDaysRaw =
    refund?.working_days ?? refund?.provider_cancel_days?.toString?.() ?? null;
  const workingDays = formatWorkingDays(
    workingDaysRaw,
    refund?.work_on_korean_holiday,
  );
  ...
  const pivotHours =
    pivotFromFees ??
    (typeof pivotHoursOverride === 'number' && pivotHoursOverride > 0
      ? pivotHoursOverride
      : typeof cancellationHours === 'number' && cancellationHours > 0
        ? cancellationHours
        : 24);
          ...
          <div className="mb-6">
            {refundType === 'CONDITIONAL' && (
              <>
                ...
                <div className="relative w-full h-12 rounded-xl overflow-visible">
                  <div
                    className={`absolute inset-y-0 left-0 w-1/2 ${theme.leftBg} flex items-center justify-center font-semibold`}
                  >
                    {ui.left}
                  </div>
                  <div
                    className={`absolute inset-y-0 right-0 w-1/2 ${theme.rightBg} flex items-center justify-center font-semibold`}
                  >
                    {ui.right}
                  </div>
                  <div
                    className={`absolute inset-y-0 left-1/2 w-0.5 ${theme.divider}`}
                  />
                </div>
              </>
            )}
            {refundType === 'AVAILABLE' && (
              <>
                ...
                <div className="relative w-full h-12 rounded-xl overflow-visible">
                  <div
                    className={`absolute inset-0 ${theme.leftBg} flex items-center justify-center font-semibold`}
                  >
                    {ui.left} (anytime)
                  </div>
                </div>
            ...
            {refundType === 'UNAVAILABLE' && (
              <>
                ...
                <div className="relative w-full h-12 rounded-xl overflow-visible">
                  <div
                    className={`absolute inset-0 ${theme.rightBg} flex items-center justify-center font-semibold`}
                  >
                    {ui.right}
                  </div>
                </div>
      ...
      <div className="space-y-5">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Cancellation and refund times are based on local time.
          </p>
        </div>
        ...
        <div className="pt-2">
          ...
          <ul className="list-disc pl-5 space-y-1 text-sm text-gray-700">
            <li>
              No cancellation fee is charged for cancellations made during the
              reservation request stage.
            </li>
            <li>
              Cancellation fees are calculated based on the product price.
            </li>
            <li>
              Cancellation deadlines may change due to local conditions; in such
              cases, separate notice will be provided.
            </li>
            <li>
              No refund for no-shows or unused bookings on the date of use.
            </li>
            <li>
              Cancellations/refunds are processed only during supplier business
              hours.
            </li>
            <li>
              Cancellations made after business hours, holidays, or weekends
              will have fees applied based on the next business day.
            </li>
          </ul>
        </div>
      </div>
