...
type TourOptionsProps = {
  ...
  onChangeLabelQuantities?: (
    optionCode: string,
    qtyByLabel: Record<string, number>,
  ) => void;
...
export function TourOptions({
  ...
  const [selectedOption, setSelectedOption] = useState<TourOption | undefined>(
    undefined,
  );
  ...
  const updateQuantity = useCallback(
    (newQuantities: Record<string, number>) => {
      const currentQuantity = Object.values(newQuantities).reduce(
        (sum, qty) => sum + qty,
        0,
      );
  ...
  const deriveTimeslotOptions = (
    ...
    const invList: any[] = Array.isArray((option as any).inventory_timeslots)
      ? (option as any).inventory_timeslots
      : [];
    const listFromInventory = invList
      .map((t: any) => ({
        ...
        labelId: t.label_id ? String(t.label_id) : undefined,
      }))
      .filter(t => t.quantity > 0);
    if (listFromInventory.length > 0) {
      return listFromInventory;
    }
    const tsList: any[] = Array.isArray((option as any).timeslots)
      ? (option as any).timeslots
      : [];
    return tsList.map((t: any) => ({
      id: String(t.code || t.id || ''),
      title: String(t.title || t.name || t.code || ''),
      quantity: 0,
      labelId: undefined,
    }));
  };
  ...
  const getOptionSummary = (
    ...
    const labels: any[] = Array.isArray((option as any).labels)
      ? (option as any).labels
      : [];
    ...
    labels.forEach((lb: any, idx: number) => {
      ...
      if (qty > 0)
        parts.push(`${String(lb?.title || lb?.name || 'Participant')} ${qty}`);
    });
      ...
      {selectedDate && options.length > 0 ? (
              ...
              <div className="flex justify-between items-start gap-4 mb-0 w-full">
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-4">
                    <h5 className="font-semibold text-gray-900 text-[16px] leading-snug truncate">
                      {option.title}
                    </h5>
                    {!cardExpanded[option.code] && (
                      <div className="text-sm text-gray-600 text-right">
                        {(() => {
                          const s = getOptionSummary(option);
                          return (
                            <>
                              {s.time && (
                      ...
                      <Button
                        ...
                        onClick={e => {
                          ...
                          if (onChangeLabelQuantities)
                            onChangeLabelQuantities(option.code || '', {});
                        }}
                      >
                        Reset
                      </Button>
                    ...
                    <>
                      ...
                      <div className="mt-4">
                        ...
                        <div className="flex flex-wrap gap-3">
                          {Array.from(
                            new Map(
                              deriveTimeslotOptions(option).map(ts => [
                                ts.title,
                                ts,
                              ]),
                            ).values(),
                            ...
                            const active =
                              selectedTimeslot[option.code || ''] === ts.id;
                            return (
                              <button
                                key={ts.id}
                                type="button"
                                onClick={e => {
                                  ...
                                  const isDifferentTimeslot =
                                    currentTimeslot &&
                                    currentTimeslot !== ts.id;
                                  ...
                                  const labels: any[] = Array.isArray(
                                    (option as any).labels,
                                  )
                                    ? (option as any).labels
                                    : [];
                                  if (
                                    labels.length > 0 &&
                                    (isDifferentTimeslot ||
                                      !labelQuantities[option.code])
                                  ) {
                                    const init: Record<string, number> = {};
                                    // default 0 for all labels
                                    labels.forEach((lb: any, i: number) => {
                                      init[
                                        String(lb?.code || lb?.id || `L${i}`)
                                      ] = 0;
                                    });
                                    setLabelQuantities(prev => ({
                                      ...prev,
                      ...
                      {selectedTimeslot[option.code || ''] && (
                        <div className="mt-4">
                          ...
                          <div className="space-y-3">
                            {(Array.isArray((option as any).labels)
                              ? (option as any).labels
                              : []
                            ).map((lab: any, idx: number) => {
                              const lCode = String(
                                lab?.code || lab?.id || `L${idx}`,
                              ).toUpperCase();
                              const current = (labelQuantities[option.code]?.[
                                lCode
                              ] ?? 0) as number;
                              const setValue = (next: number) => {
                                setLabelQuantities(prev => {
                                  ...
                                  const total = Object.values(cur).reduce(
                                    (s, n) => s + (n as number),
                                    0,
                                  );
                                  const newQuantities = {
                              ...
                              let unit = getLabelPrice
                                ? getLabelPrice(option.code || '', lCode)
                                : 0;
                              if (
                                !unit &&
                                typeof (lab as any)?.price === 'number'
                              )
                                unit = Number((lab as any).price);
                              if (
                                !unit &&
                                typeof (lab as any)?.net_price_currency ===
                                  'number'
                              )
                                unit = Number((lab as any).net_price_currency);
                              const isLoading =
                                (lab as any)?.isLoading === true;
                              const maxText =
                                typeof lab?.per_max === 'number' &&
                                lab.per_max > 100
                                  ? '100+'
                                  : String(lab?.per_max ?? 0);
                              const minNum =
                                typeof lab?.per_min === 'number'
                                  ? lab.per_min
                                  : 0;
                              return (
                                <div
                      ...
                      {selectedDate &&
                        (() => {
                          ...
                          const totalSel = Object.values(curMap).reduce(
                            (s, n) => s + Number(n || 0),
                            0,
                          );
                          return totalSel > 0;
                          ...
                          <div className="mt-4">
                            ...
                            <div className="font-semibold text-gray-900 text-[16px]">
                              {(() => {
                                const curMap =
                                  labelQuantities[option.code] || {};
                                const hasLoadingLabel = Object.entries(
                                  curMap,
                                ).some(([lCode, qty]) => {
                                ...
                                let sum = 0;
                                Object.entries(curMap).forEach(
                                  ([lCode, qty]) => {
                                    const code = String(lCode).toUpperCase();
                                    let u = getLabelPrice
                                      ? getLabelPrice(option.code || '', code)
                                      : 0;
                                    if (!u) {
                                      const lab = (
                                        Array.isArray((option as any).labels)
                                      ...
                                      if (lab) {
                                        if (typeof lab?.price === 'number')
                                          u = Number(lab.price);
                                        else if (
                                          typeof lab?.net_price_currency ===
                                          'number'
                                        )
                                          u = Number(lab.net_price_currency);
                                      }
                                    }
        ...
        <div className="text-center py-12 text-gray-500">
          <p className="text-base">
            No available options for the selected date.
          </p>
