...
export function TourImageGallery({ images, title }: TourImageGalleryProps) {
  ...
  useEffect(() => {
    if (thumbnailListRef.current && currentIndex < maxThumbnails - 1) {
      const thumbnailElement = thumbnailListRef.current.children[
        currentIndex
      ] as HTMLElement;
      if (thumbnailElement) {
  ...
  useEffect(() => {
    if (isModalOpen) {
      const modalThumbnails = document.querySelector(
        '.modal-thumbnails',
      ) as HTMLElement;
      if (modalThumbnails) {
        const thumbnailElement = modalThumbnails.children[
          modalIndex
        ] as HTMLElement;
        if (thumbnailElement) {
          thumbnailElement.scrollIntoView({
      ...
      <div className="flex gap-3 md:gap-4 flex-col md:flex-row">
        ...
        <div className="flex-1 relative">
          <div className="relative aspect-video sm:aspect-[3/2] md:aspect-[4/3] rounded-lg overflow-hidden">
            <img
              src={
                images[currentIndex].includes('http')
                  ? images[currentIndex]
                  : `https://images.unsplash.com/${images[currentIndex]}?auto=format&fit=crop&w=800&h=600&q=80`
              }
        ...
        <div className="w-full md:w-24 md:aspect-[4/3]">
          <div
            ...
            className="flex md:flex-col gap-2 md:h-full md:justify-between overflow-x-auto pb-2 md:pb-0 scrollbar-hide px-1 pt-1"
          >
              ...
              >
                <img
                  src={
                    image.includes('http')
                      ? image
                      : `https://images.unsplash.com/${image}?auto=format&fit=crop&w=120&h=120&q=80`
                  }
            ...
            {images.length >= maxThumbnails && (
              ...
              >
                <img
                  src={
                    images[maxThumbnails - 1].includes('http')
                      ? images[maxThumbnails - 1]
                      : `https://images.unsplash.com/${images[maxThumbnails - 1]}?auto=format&fit=crop&w=120&h=120&q=80`
                  }
            ...
            <div className="p-4">
              ...
              <div className="relative mb-4">
                <div className="aspect-[4/3] bg-gray-100 rounded-lg overflow-hidden">
                  <img
                    src={
                      images[modalIndex].includes('http')
                        ? images[modalIndex]
                        : `https://images.unsplash.com/${images[modalIndex]}?auto=format&fit=crop&w=800&h=600&q=80`
                    }
                  ...
                  >
                    <img
                      src={
                        image.includes('http')
                          ? image
                          : `https://images.unsplash.com/${image}?auto=format&fit=crop&w=80&h=80&q=80`
                      }
                      alt={`${title} thumbnail ${index + 1}`}
                      className="w-full h-full object-cover"
                    />
