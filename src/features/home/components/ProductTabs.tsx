 ...
 */
export function ProductTabs({ categories, title = '추천 상품' }: ProductTabsProps) {
      ...
      <ResponsiveContainer>
        <div className="mt-2">
          ...
          <Tabs defaultValue={tabGroups[0]?.key ?? 'cat-0'} className="w-full">
            ...
            {tabGroups.map(tab => (
              <TabsContent key={tab.key} value={tab.key} className="mt-4 sm:mt-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  {tab.items?.slice(0, 4).map((product: any) => (
                    <Link key={product.id} href={product.href || `/activity/product/${product.id}`} className="block">
                            ...
                            {typeof product.rating === 'number' && product.rating > 0 && 
                             typeof product.reviewCount === 'number' && product.reviewCount > 0 && (
                              <div className="flex items-center text-[11px] sm:text-xs text-yellow-600 mb-1.5 sm:mb-2">
                                ...
                                {product.rating?.toFixed ? product.rating.toFixed(1) : product.rating} ({product.reviewCount})
                              </div>
                            )}
