...
export function SectionFourByOne({ title, defaultItems }: Props) {
  const query = useSectionProducts({
      ...
      <CarouselContent className="-ml-2 md:-ml-4">
        {items.map(p => {
          const productItem: ProductItem = {
            ...
            images:
              (p as any).images || ((p as any).image ? [(p as any).image] : []),
            price:
              typeof (p as any).price === 'string'
                ? Number((p as any).price)
                : (p as any).price,
            originalPrice:
              typeof (p as any).originalPrice === 'string'
                ? Number((p as any).originalPrice)
                : (p as any).originalPrice,
            discountRate:
              typeof (p as any).discountRate === 'string'
                ? Number((p as any).discountRate)
                : (p as any).discountRate,
