...
export default function HomePageClient({
  ...
  // 섹션별 데이터 추출
  const regionsForCarousel =
    sections?.find(s => s.templateId === 'TV_TM_CAROUSEL' && 'regions' in s)
      ?.regions ?? regions;
  const bannersForLine =
    sections?.find(
      s => s.templateId === 'TV_PC_IV_LINE_BANNER_A' && 'banners' in s,
    )?.banners ?? banners;
  const bestTabCats =
    sections?.find(s => s.templateId === 'TV_TAB_BSTP' && 'categories' in s)
      ?.categories ?? categories.slice(0, 4);
  const fourByOneCats =
    (sections
      ?.filter(s => s.templateId === 'TV_PC_TM_PRODUCT_4X1' && 'category' in s)
      .map(s => (s as any).category) as Category[]) ?? categories.slice(0, 3);
  const twoGridCats =
    sections?.find(s => s.templateId === 'TV_TAB_TWOGRID' && 'categories' in s)
  ...
  const selectedRegion =
    regionsForCarousel.find(r => r.id === activeRegionId) ||
    regionsForCarousel[0];
