...
export function StickyBox({ children, topOffset = 112, enableBelow = 1024, boundarySelector = '.booking-container', triggerSelector, debug = false }: StickyBoxProps) {
  ...
  const compute = useCallback(() => {
    ...
    const boundary = (boundarySelector ? (container.closest(boundarySelector) as HTMLElement | null) : null) || (boundarySelector ? (document.querySelector(boundarySelector) as HTMLElement | null) : null) || container
    ...
    let triggerBottom = 0
    if (triggerSelector) {
      const trig = document.querySelector(triggerSelector) as HTMLElement | null
      if (trig) {
  ...
  }, [compute, debug, topOffset, enableBelow, boundarySelector, triggerSelector])
  ...
  return (
    <div ref={containerRef} style={{ position: 'relative' }} data-stickybox={debug ? 'debug' : undefined}>
