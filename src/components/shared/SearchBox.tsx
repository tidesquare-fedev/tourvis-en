...
export default function SearchBox({
  channelId = 3,
  host,
  placeholder = 'Search tours, regions, categories',
  className,
}: SearchBoxProps) {
  ...
  const resolvedHost = useMemo(() => {
    if (host) return host;
    if (typeof process !== 'undefined' && process.env.NEXT_PUBLIC_SEARCH_HOST)
      return String(process.env.NEXT_PUBLIC_SEARCH_HOST);
  ...
  const onSubmit = (e?: React.FormEvent) => {
    ...
    if (!q) return;
    router.push(
      `/activity/search?q=${encodeURIComponent(q)}&channel_id=${channelId}`,
    );
  };
