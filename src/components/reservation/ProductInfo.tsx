      ...
      <CardContent>
        <div className="flex items-start space-x-4">
          ...
          <div className="flex-1">
            <h3 className="font-semibold text-lg mb-2">{reservation.tour?.title || reservation.tourTitle}</h3>
            <div className="space-y-1 text-sm text-gray-600">
              ...
              <div className="flex justify-between">
                ...
                <span>{reservation.travelerText || `${reservation.participants} Travelers`}</span>
              </div>
              {!hidePrice && (
                <>
                  <div className="flex justify-between">
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              <Link href={`/tour/${reservation.tour?.id || '1'}`} className="flex-1 min-w-0">
                        ...
                        <div className="space-y-3 sm:space-y-4 border-t pt-4">
                            ...
                            <div key={idx} className="border rounded-lg p-2 sm:p-3">
                              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-2">
                                <span className="font-medium text-sm sm:text-base break-words">
                                  {s.optionTitle}{s.timeslotTitle ? ` • ${s.timeslotTitle}` : ''}
                                </span>
                              </div>
                          ...
                          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center pt-3 border-t gap-1 sm:gap-2">
                            ...
                            <div className="text-right">
                              <span className="text-base sm:text-lg font-bold">${formatNum(reservation.totalAmount)}</span>
                              <br />
                  ...
                  <div className="px-4 pb-6 space-y-6 max-h-[70vh] overflow-y-auto">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <p className="text-gray-600">{reservation.dateTimeText || reservation.date}</p>
                      </div>
                    </div>
                    ...
                    <div className="border-t pt-4">
                      ...
                      <div className="mt-2 space-y-1 text-sm text-gray-600">
                        ...
                        <p><strong>Participants:</strong> {reservation.travelerText || `${reservation.participants} Travelers`}</p>
                      </div>
                    </div>

                    {/* Product Information */}
                    <div className="border-t pt-4">
                      <span className="font-semibold text-sm">Product Information:</span>
                      <div className="mt-2 space-y-2 text-sm">
                        <div>
                          <strong>Tour Title:</strong> {reservation.tour?.title || reservation.tourTitle}
                        </div>
                        <div>
                          <strong>Price per person:</strong> ${formatNum(reservation.tourPrice)}
                        </div>
                        <div>
                          <strong>Total Amount:</strong> ${formatNum(reservation.totalAmount)}
                        </div>
                        {reservation.selections && reservation.selections.length > 0 && (
                          <div>
                            ...
                            <ul className="list-disc list-inside ml-2 text-gray-600 mt-1">
                              {reservation.selections.map((s, idx) => (
                                <li key={idx}>
                                  {s.optionTitle}{s.timeslotTitle ? ` • ${s.timeslotTitle}` : ''} - ${formatNum(s.subtotal)}
                                </li>
                              ))}
                            </ul>
                          </div>
