...
export function useTnaOptions(
  productId: string,
  args: {
    calendarType: 'DATE' | 'PERIOD';
    selectedDate?: Date;
    range?: DateRange;
  },
) {
  ...
  const dateStr = useMemo(
    () => (selectedDate ? formatLocalDate(new Date(selectedDate)) : undefined),
    [selectedDate],
  );
  const startStr = useMemo(
    () => (range?.from ? formatLocalDate(new Date(range.from)) : undefined),
    [range?.from],
  );
  const endStr = useMemo(
    () => (range?.to ? formatLocalDate(new Date(range.to)) : startStr),
    [range?.to, startStr],
  );
  ...
  const optionsQuery = useQuery({
    ...
    queryFn: async () => {
      if (isDateType && dateStr) {
        ...
        const bundle = await nextTnaApi.bundle({
  ...
  const createDynamicPricePayload = (
    selectedOption: any,
    selectedTimeslot: any,
    count: number,
    selectedLabelId?: string,
  ): DynamicPriceRequest | null => {
    if (!selectedOption || !selectedTimeslot || !dateStr) {
      return null;
    }
