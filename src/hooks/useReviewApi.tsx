...
export function useReviews(prodCd: string, ctx?: BrandCat) {
  ...
  const isError =
    Boolean(countQuery.error || listQuery.error) ||
    (countQuery.data as any)?.success === false ||
    (listQuery.data as any)?.success === false;

  const error =
    (countQuery.error as Error) || (listQuery.error as Error) || undefined;
