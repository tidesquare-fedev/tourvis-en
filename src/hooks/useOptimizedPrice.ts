...
export function useOptimizedPrice(options: UseOptimizedPriceOptions = {}) {
  ...
  // 가격 요청 debounce
  const { debouncedPriceParams, isCalculating: isDebouncing } = usePriceDebounce(
    priceRequest, 
    debounceDelay
  )
  ...
  // 가격 조회 함수
  const fetchPrice = useCallback(async (request: PriceRequest) => {
    try {
      ...
      if (result?.success === false) {
        throw createError(ERROR_CODES.DATA_NOT_FOUND, result.error || '가격 정보를 불러올 수 없습니다.')
      }
      ...
      const errorMessage = err instanceof Error ? err.message : '가격 조회 중 오류가 발생했습니다.'
  ...
  const fetchPriceImmediate = useCallback(async (request: PriceRequest) => {
    return fetchPrice(request)
  }, [fetchPrice])

  // 가격 요청 유효성 검사
  const isValidRequest = useCallback((request: PriceRequest | null): boolean => {
  ...
  // 현재 상태
  const isLoading = isCalculating || isDebouncing || dynamicPriceMutation.isPending
  ...
  const formattedPrice = useMemo(() => {
    ...
    return {
      ...
      original: originalPrice ? `${currency} ${originalPrice.toLocaleString()}` : null,
