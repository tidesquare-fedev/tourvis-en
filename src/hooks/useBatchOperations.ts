...
import { homeApiClient, activityApiClient, reviewApiClient } from '@/lib/api/optimized-api'
...
export function useBatchPrefetch() {
  ...
  const prefetchHomeData = useCallback(async () => {
    const promises = [
      ...
      queryClient.prefetchQuery({
        ...
        queryFn: () => homeApiClient.getSectionProducts({
   ...
   */
  const prefetchProductDetail = useCallback(async (productId: string) => {
    const promises = [
      // 상품 기본 정보
      queryClient.prefetchQuery({
      ...
      queryClient.prefetchQuery({
        ...
        queryFn: () => reviewApiClient.getReviewCount({
          brand: 'TOURVIS',
          prodCat: 'ACTIVITY',
          prodCd: productId,
        }),
   ...
   * 검색 결과 프리페치
   */
  const prefetchSearchResults = useCallback(async (searchParams: {
    providerIds: string
    keyword?: string
    count?: number
  }) => {
    await queryClient.prefetchQuery({
      queryKey: queryKeys.activity.products(searchParams.providerIds, searchParams.keyword),
   ...
   * 관련 상품 프리페치
   */
  const prefetchRelatedProducts = useCallback(async (productId: string, category?: string) => {
    const promises = [
      // 같은 카테고리 상품들
      queryClient.prefetchQuery({
        queryKey: queryKeys.activity.products('ALL', category),
        queryFn: () => activityApiClient.searchProducts({
          providerIds: 'ALL',
          keyword: category,
          count: 12,
        }),
...
export function useBatchCacheInvalidation() {
   ...
   * 상품 관련 모든 캐시 무효화
   */
  const invalidateProductRelated = useCallback(async (productId: string) => {
    const patterns = [
   ...
   * 검색 관련 캐시 무효화
   */
  const invalidateSearchRelated = useCallback(async (providerIds: string) => {
...
export function useBatchDataLoader() {
   ...
   * 여러 상품 정보를 한 번에 로드
   */
  const loadMultipleProducts = useCallback(async (productIds: string[]) => {
   ...
   * 여러 리뷰 정보를 한 번에 로드
   */
  const loadMultipleReviews = useCallback(async (productIds: string[]) => {
    const promises = productIds.map(id => 
      Promise.all([
        queryClient.fetchQuery({
        ...
        queryClient.fetchQuery({
          ...
          queryFn: () => reviewApiClient.getReviews({
            brand: 'TOURVIS',
            prodCat: 'ACTIVITY',
            prodCd: id,
            limit: 5,
   ...
   * 섹션별 상품들을 한 번에 로드
   */
  const loadSectionProducts = useCallback(async (sections: Array<{
    templateId: string
    title?: string
    limit?: number
  }>) => {
...
export function useSmartPrefetch() {
   ...
   */
  const onProductHover = useCallback(async (productId: string, category?: string) => {
    // 상품 상세 정보 프리페치
   ...
   */
  const onSearchInput = useCallback(async (keyword: string, providerIds: string) => {
    if (keyword.length >= 2) {
      // 검색 결과 프리페치
      await queryClient.prefetchQuery({
   ...
   * 페이지 진입 시 백그라운드 프리페치
   */
  const onPageEnter = useCallback(async (pageType: 'home' | 'product' | 'search', params?: any) => {
    switch (pageType) {
      case 'home':
        // 홈 페이지 관련 데이터 프리페치
        break
        ...
        if (params?.providerIds) {
          ...
          await Promise.allSettled(
            popularKeywords.map(keyword =>
              queryClient.prefetchQuery({
                queryKey: queryKeys.activity.products(params.providerIds, keyword),
                queryFn: () => activityApiClient.searchProducts({
