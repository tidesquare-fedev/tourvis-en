...
import { homeApiClient, activityApiClient, reviewApiClient, tnaApiClient } from '@/lib/api/optimized-api'
...
export function useHomeData() {
  return useQuery({
    ...
    select: (data) => {
      if (!data?.success) {
        throw createError(ERROR_CODES.DATA_NOT_FOUND, '홈 페이지 데이터를 불러올 수 없습니다.')
      }
      return data.data
    },
  })
...
export function useProductDetail(productId: string) {
  return useQuery({
    ...
    select: (data) => {
      if (!data?.success) {
        throw createError(ERROR_CODES.DATA_NOT_FOUND, '상품 정보를 불러올 수 없습니다.')
      }
      return data.data
    },
  })
...
export function useTnaOptionsDateType(productId: string, date: string) {
  return useQuery({
    ...
    select: (data) => {
      if (!data?.success) {
        throw createError(ERROR_CODES.DATA_NOT_FOUND, '상품 옵션을 불러올 수 없습니다.')
      }
      return data.data
    },
  })
...
export function usePrefetch() {
  ...
  const prefetchReviews = useCallback(async (params: {
    brand: string
    prodCat: string
    prodCd: string
  }) => {
    await Promise.all([
      queryClient.prefetchQuery({
        queryKey: queryKeys.review.count(params.prodCd, params.brand, params.prodCat),
        queryFn: () => reviewApiClient.getReviewCount(params),
        ...queryOptions.dynamic,
      }),
      queryClient.prefetchQuery({
        queryKey: queryKeys.review.list(params.prodCd, params.brand, params.prodCat),
        queryFn: () => reviewApiClient.getReviews(params),
        ...queryOptions.dynamic,
      }),
    ])
...
export function useQueryState() {
  ...
  const hasError = useMemo(() => {
    return queryClient.getQueriesData({}).some(([, data]) => 
