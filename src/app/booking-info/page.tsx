...
function BookingInfoContent() {
  ...
  const tour = storedObj
    ? {
        ...
        price: Math.round(
          Number(storedObj.totalAmount || 0) /
  ...
  const selections = Array.isArray(storedObj?.selections)
    ? (storedObj.selections as Array<{
        optionTitle: string;
  ...
  const dateText = (() => {
    ...
    return storedObj?.date
      ? new Date(storedObj.date).toLocaleDateString()
      : formData.date
  ...
  const selectedTimes = Array.from(
    new Set(
      selections.map(s => String(s.timeslotTitle || '').trim()).filter(Boolean),
    ),
  );
  const dateTimeText =
    selectedTimes.length > 0
      ? `${dateText} • ${selectedTimes.join(', ')}`
      : `${dateText}${storedObj?.timeslotTitle ? ` • ${storedObj.timeslotTitle}` : ''}`;
  ...
  const total = (() => {
    ...
    return selections.length > 0
      ? selections.reduce(
          (sum, selection) => sum + (selection.subtotal || 0),
          0,
        )
      : storedObj?.totalAmount ||
          tour.price * (formData.adults + formData.children);
  })();
  ...
  const [isTicketCountryModalOpen, setIsTicketCountryModalOpen] =
    useState(false);
  ...
  const setTicketField = (field: string, value: any) =>
    setTicketUserData(p => ({ ...p, [field]: value }));
  ...
  const getStepIcon = (step: number) => {
    ...
    return (
      <div className="w-6 h-6 bg-gray-300 text-gray-600 rounded-full flex items-center justify-center text-sm">
        {step}
      </div>
    );
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 sm:py-4">
          <div className="flex items-center gap-3 sm:gap-6">
            ...
            <Link href="/" className="flex items-center">
              <span
                className="logo h-6 sm:h-8 w-24 sm:w-28"
                role="img"
                aria-label="TOURVIS"
              />
            </Link>
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 py-4 sm:py-8">
                          ...
                          {selections.length > 0 ? (
                            ...
                            <>
                              <div className="flex justify-between items-center">
                                <span className="font-medium text-sm sm:text-base break-words">
                                  {storedObj?.optionTitle || tour.title}
                                </span>
                              </div>
                              <div className="flex justify-between items-center">
                          ...
                          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center pt-3 border-t gap-1 sm:gap-2">
                            ...
                            <div className="text-right">
                              ...
                              <span className="text-xs sm:text-sm font-normal text-gray-600">
                                ${formatNum(tour.price)} per person
                              </span>
                            </div>
                          </div>
                        </div>
        ...
        <Card className="mb-6">
          <CardHeader
            ...
            onClick={() => isStepAccessible(1) && setActiveStep(1)}
          >
            <CardTitle className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                {getStepIcon(1)} Contact details
              </div>
              {isStepCompleted(1) && (
                <Button
                  variant="outline"
                  ...
                  onClick={e => {
                    e.stopPropagation();
                    setActiveStep(1);
                    setCompletedSteps(s => s.filter(x => x !== 1));
                  }}
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1 bg-green-50 p-2 rounded">
                  Your booking confirmation will be sent to{' '}
                    ...
                    <div className="space-y-2 max-h-60 overflow-y-auto">
                      {countryCodes.map(c => (
                        ...
                        >
                          ...
                          <span
                            className={`flex-1 text-left ${formData.countryCode === c.code ? 'text-white' : 'text-gray-900'}`}
                          >
        ...
        <Card className="mb-6">
          <CardHeader
            className={`cursor-pointer ${isStepAccessible(2) ? '' : 'opacity-50'}`}
            onClick={() => isStepAccessible(2) && setActiveStep(2)}
          >
            <CardTitle className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                {getStepIcon(2)} Activity details
              </div>
            </CardTitle>
          </CardHeader>
          {isStepActive(2) && (
            <CardContent className="space-y-6">
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input
                      ...
                      onBlur={e =>
                        validateField('firstName', e.target.value, 'ticket')
                      }
                      disabled={sameAsTraveler}
                      required
                    />
                    {errors.ticketFirstName && (
                      <p className="text-sm text-red-500 mt-1">
                        {errors.ticketFirstName}
                      </p>
                    )}
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onBlur={e =>
                        validateField('lastName', e.target.value, 'ticket')
                      }
                      disabled={sameAsTraveler}
                      required
                    />
                    {errors.ticketLastName && (
                      <p className="text-sm text-red-500 mt-1">
                        {errors.ticketLastName}
                      </p>
                    )}
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={e => setTicketField('email', e.target.value)}
                    onBlur={e =>
                      validateField('email', e.target.value, 'ticket')
                    }
                    disabled={sameAsTraveler}
                    required
                  />
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <div className="hidden sm:block">
                      <Select
                        ...
                        onValueChange={v => setTicketField('countryCode', v)}
                        disabled={sameAsTraveler}
                      >
                        <SelectTrigger className="w-48">
                          <SelectValue />
                        ...
                        <SelectContent>
                          {countryCodes.map(c => (
                            ...
                            >
                              <span className="mr-3">{c.flag}</span>
                              <span className="w-20 inline-block mr-6">
                                {c.code}
                              </span>
                              <span>
                                {c.country} ({c.code})
                    ...
                    <div className="sm:hidden">
                      ...
                      >
                        <span className="text-sm">
                          {
                            countryCodes.find(
                              c => c.code === ticketUserData.countryCode,
                            )?.flag
                          }{' '}
                          {ticketUserData.countryCode}
                        </span>
                      </Button>
                    </div>
                    ...
                    <Input
                      ...
                      onBlur={e =>
                        validateField('phone', e.target.value, 'ticket')
                      }
                      disabled={sameAsTraveler}
                      className="flex-1"
                      required
                  ...
                  <Dialog
                    open={isTicketCountryModalOpen}
                    onOpenChange={setIsTicketCountryModalOpen}
                  >
                    <DialogContent className="w-[95vw] max-w-md">
                      <DialogHeader>
                        <DialogTitle>Select Country Code</DialogTitle>
                      ...
                      <div className="space-y-2 max-h-60 overflow-y-auto">
                        {countryCodes.map(c => (
                          ...
                          >
                            <span className="text-lg mr-3">{c.flag}</span>
                            <span
                              className={`flex-1 text-left ${ticketUserData.countryCode === c.code ? 'text-white' : 'text-gray-900'}`}
                            >
                              {c.country} ({c.code})
                            </span>
                          </Button>
              ...
              <div className="space-y-4 border-t pt-4">
                <div className="space-y-2">
                  <p className="text-sm text-gray-600">
                    • Be careful to write accurately when entering your
                    reservation.
                  </p>
                  <p className="text-sm text-gray-600">
                    • Please provide accurate contact information for smooth
                    reservation.
                  </p>
                  <p className="text-sm text-gray-600">
                    • Reservation information cannot be changed arbitrarily
                    after reservation.
                  </p>
                </div>
                <div>
            ...
            <CardContent>
              <div className="space-y-4 text-sm">
                <div>
                  ...
                  <p className="text-gray-600">
                    {ticketUserData.firstName} {ticketUserData.lastName}
                  </p>
                  <p className="text-gray-600">{ticketUserData.email}</p>
                  <p className="text-gray-600">
                    {ticketUserData.countryCode} {ticketUserData.phone}
                  </p>
                </div>
        ...
        <Card className="mb-6">
          <CardHeader
            className={`cursor-pointer ${isStepAccessible(3) ? '' : 'opacity-50'}`}
            onClick={() => isStepAccessible(3) && setActiveStep(3)}
          >
            <CardTitle className="flex items-center gap-2">
              {getStepIcon(3)} Payment details
            </CardTitle>
          </CardHeader>
          {isStepActive(3) && (
            <CardContent className="space-y-6">
