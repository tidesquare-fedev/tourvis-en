        ...
        if (saved) {
          try {
            ...
            if (reservationData.tour?.id) {
              console.log(
                'Using reservation data for tour ID:',
                reservationData.tour.id,
              );

              // 예약 데이터에 포함된 상품 상세 정보 사용
              if (reservationData.productDetails) {
                console.log(
                  'Using stored product details:',
                  reservationData.productDetails,
                );
      ...
      <div className="min-h-screen bg-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
          <div className="text-center py-12">
            ...
            <Link
              href="/"
              className="inline-block bg-[#01c5fd] text-white px-6 py-3 rounded-lg hover:bg-[#01b7ea] transition-colors"
            >
              Go to Home
            </Link>
          </div>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center text-base sm:text-lg">
                  <Clock className="w-4 h-4 sm:w-5 sm:h-5 mr-2" /> Reservation
                  Info
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-3">
                    ...
                    {(productDetails.basic?.duration ||
                      productDetails.basic?.duration_unit) && (
                      <div>
                        ...
                        <p className="text-sm text-gray-600 ml-5">
                          {productDetails.basic.duration
                            ? `${Math.floor(productDetails.basic.duration / 60)} hours`
                    ...
                    {(productDetails.basic?.meeting_point ||
                      productDetails.basic?.meeting_point_address) && (
                      <div>
                        <span className="font-semibold text-sm flex items-center">
                          <MapPin className="w-4 h-4 mr-1" /> Meeting Point:
                        </span>
                        <p className="text-sm text-gray-600 ml-5">
                          {productDetails.basic.meeting_point ||
                            productDetails.basic.meeting_point_address ||
                    ...
                    {productDetails.basic?.meeting_time && (
                      <div>
                        <span className="font-semibold text-sm flex items-center">
                          <Clock className="w-4 h-4 mr-1" /> Meeting Time:
                        </span>
                        <p className="text-sm text-gray-600 ml-5">
                          {productDetails.basic.meeting_time}
                        </p>
                ...
                {productDetails.basic?.included &&
                  Array.isArray(productDetails.basic.included) &&
                  productDetails.basic.included.length > 0 && (
                    <div className="border-t pt-4">
                      ...
                      <ul className="list-disc list-inside ml-5 space-y-1 text-sm text-gray-600">
                        {productDetails.basic.included.map(
                ...
                {productDetails.basic?.excluded &&
                  Array.isArray(productDetails.basic.excluded) &&
                  productDetails.basic.excluded.length > 0 && (
                    <div className="border-t pt-4">
                      <span className="font-semibold text-sm flex items-center mb-2">
                        <XCircle className="w-4 h-4 mr-1 text-red-600" />{' '}
                        <span style={{ color: '#ff00cc' }}>Not</span> Included:
                      </span>
                      <ul className="list-disc list-inside ml-5 space-y-1 text-sm text-gray-600">
                        {productDetails.basic.excluded.map(
                          (item: string, index: number) => (
                            <li key={index}>{item}</li>
                          ),
                        )}
                ...
                {productDetails.basic?.requirements &&
                  Array.isArray(productDetails.basic.requirements) &&
                  productDetails.basic.requirements.length > 0 && (
                    <div className="border-t pt-4">
                      <span className="font-semibold text-sm flex items-center mb-2">
                      ...
                      <ul className="list-disc list-inside ml-5 space-y-1 text-sm text-gray-600">
                        {productDetails.basic.requirements.map(
                          (item: string, index: number) => (
                            <li key={index}>{item}</li>
                          ),
                        )}
