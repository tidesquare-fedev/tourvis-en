...
const mapProductsToItems = (list: any[]): any[] => {
  return (Array.isArray(list) ? list : []).map((p: any) => {
    ...
    const rating =
      Number(detail.tntReviewRating ?? detail?.review?.reviewScore ?? 0) ||
      undefined;
    const reviewCount =
      Number(detail.tntReviewCount ?? detail?.review?.reviewCount ?? 0) ||
      undefined;
    return {
...
export async function POST(req: Request) {
  try {
    ...
    const root: any = data;
    const categoryList: any[] = Array.isArray(root.categoryList)
      ? root.categoryList
          .slice()
    ...
    for (const entry of categoryList) {
      ...
      if (tpl === 'TV_TAB_BSTP') {
        if (Array.isArray(d?.themeList)) {
          ...
          sections.push({
            ...
            categories,
            title: String(
              d?.name ?? entry?.name ?? d?.title ?? 'Best Products',
            ),
          });
        } else if (Array.isArray(d?.productList)) {
          const items = mapProductsToItems(d.productList);
          const catTitle = String(
            d?.title ?? d?.name ?? entry?.name ?? 'Best Products',
          );
          sections.push({
            templateId: 'TV_TAB_BSTP',
            categories: [{ title: catTitle, items }],
            title: String(d?.name ?? entry?.name ?? 'Best Products'),
          });
        }
        continue;
      }
      ...
      if (tpl === 'TV_PC_TM_PRODUCT_4X1') {
        if (Array.isArray(d?.productList)) {
          ...
          sections.push({
            templateId: 'TV_PC_TM_PRODUCT_4X1',
      ...
      if (tpl === 'TV_TAB_TWOGRID') {
        ...
        sections.push({
          ...
          categories,
          title: String(d?.name ?? entry?.name ?? d?.title ?? ''),
        });
        continue;
      }
    ...
    if (candidates.length === 0) {
      return NextResponse.json(
        { success: true, data: { items: [] }, meta: { count: 0 } },
        { status: 200 },
      );
    }
    ...
    if (typeof title === 'string') {
      chosen = candidates.find(s => {
        if (s.templateId === 'TV_PC_TM_PRODUCT_4X1')
          return s.category.title === title;
        if (s.templateId === 'TV_TAB_BSTP' || s.templateId === 'TV_TAB_TWOGRID')
          return s.categories.some(c => c.title === title);
        return false;
      });
    }
    if (!chosen) {
      chosen =
        candidates[
          Math.max(0, Math.min(candidates.length - 1, index >= 0 ? index : 0))
        ];
    }
    ...
    if (chosen.templateId === 'TV_PC_TM_PRODUCT_4X1') {
      ...
      chosen.templateId === 'TV_TAB_BSTP' ||
      chosen.templateId === 'TV_TAB_TWOGRID'
    ) {
      const cat =
        typeof title === 'string'
          ? chosen.categories.find(c => c.title === title)
          : chosen.categories[0];
      items = cat?.items || [];
    }
    ...
    return NextResponse.json(
      {
        success: true,
        data: { items },
        meta: { count: Array.isArray(items) ? items.length : 0 },
      },
      { status: 200 },
    );
  } catch (e: unknown) {
    return NextResponse.json(
      {
        success: false,
        error: 'Unexpected error while fetching section products',
        code: 'INTERNAL_ERROR',
      },
      { status: 500 },
    );
