...
export async function POST(req: NextRequest, { params }: { params: { productId: string } }) {
  ...
  try {
    ...
    try {
      ...
      if (changed) {
        try {
          const data2 = await getProductPricePeriodTypeV2(params.productId, fallbackBody)
          return Response.json({ success: true, data: data2, meta: { fallback: 'compact-date' } })
        } catch (e2: unknown) {
          const status = getStatusFromError(e2)
          return Response.json({ success: false, error: 'Failed to load price (period-type)', code: 'PRICE_PERIOD_FAILED' }, { status })
        }
      }
    ...
    return Response.json({ success: false, error: 'Failed to load price (period-type)' }, { status: 500 })
  }
}
