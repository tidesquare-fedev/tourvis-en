    ...
    if (date) {
      try {
        ...
        try {
          ...
          const data2 = await getProductOptionsDateTypeV2(
            params.productId,
            compact,
          );
          return Response.json({
          ...
          try {
            const periodData = await getProductOptionsPeriodTypeV2(
              params.productId,
            );
            return Response.json({
              success: true,
              data: periodData,
              meta: { fallback: 'period-type' },
            });
          } catch (e3) {
            return Response.json(
              {
                success: false,
                error: 'Failed to load options (date-type/period-type)',
              },
              { status: 500 },
            );
          }
