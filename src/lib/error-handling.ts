...
export const ERROR_MESSAGES: Record<ErrorCode, string> = {
  ...
  [ERROR_CODES.API_RATE_LIMIT]: '요청이 너무 많습니다. 잠시 후 다시 시도해주세요.',
  ...
  [ERROR_CODES.COMPONENT_RENDER_ERROR]: '화면을 불러오는 중 오류가 발생했습니다.',
 ...
 */
export function logError(error: Error | AppErrorClass, additionalContext?: ErrorContext): void {
  const errorInfo = {
    name: error.name,
...
export async function safeExecute<T>(
  ...
  try {
    ...
    const appError = error instanceof AppErrorClass 
      ? error 
      : createError(errorCode, error instanceof Error ? error.message : 'Unknown error', error, context)
 ...
 */
export function handleApiError(response: Response, context?: ErrorContext): AppErrorClass | null {
  ...
  return createError(errorCode, message, { status: response.status, statusText: response.statusText }, context)
}
