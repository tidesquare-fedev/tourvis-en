import { ApiResponse, ApiRequestConfig, AppError, ErrorType } from '../types/common'
...
export class ApiClient {
  ...
  async request<T = any>(
    ...
    try {
      ...
      if (!response.ok) {
        throw this.createError(response.status, data?.error || 'Request failed', data)
      }
   ...
   * POST 요청
   */
  async post<T = any>(endpoint: string, body?: any, config?: Omit<ApiRequestConfig, 'method'>) {
   ...
   * PUT 요청
   */
  async put<T = any>(endpoint: string, body?: any, config?: Omit<ApiRequestConfig, 'method'>) {
   ...
   * PATCH 요청
   */
  async patch<T = any>(endpoint: string, body?: any, config?: Omit<ApiRequestConfig, 'method'>) {
   ...
   * 에러 생성
   */
  private createError(status: number, message: string, details?: any): AppError {
