 ...
 */
export function getCarouselItemClassName(basis: string = 'basis-3/4 sm:basis-1/2 md:basis-1/3 lg:basis-1/4'): string {
 ...
 * @returns 인디케이터 클래스명
 */
export function getCarouselIndicatorClassName(isActive: boolean, className: string = ''): string {
  const baseClasses = 'h-2.5 w-2.5 rounded-full transition-all'
  const activeClasses = isActive ? 'bg-blue-600 w-5' : 'bg-gray-300 hover:bg-gray-400'
 ...
 */
export function getCarouselIndicatorsContainerClassName(className: string = ''): string {
 ...
 * @returns 스크롤 스냅 배열
 */
export function getCarouselScrollSnaps(itemCount: number, slidesToScroll: number = 1): number[] {
 ...
 * @returns 현재 인덱스
 */
export function getCarouselCurrentIndex(scrollSnaps: number[], currentScroll: number): number {
 ...
 * @returns 기본값이 적용된 옵션
 */
export function getCarouselDefaultOptions(options: Partial<CarouselOptions> = {}): CarouselOptions {
  return {
    align: 'start',
    slidesToScroll: 1,
    loop: false,
