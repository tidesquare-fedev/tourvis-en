...
export async function getProductDatesV2(productId: string, startDate?: string) {
  const url = new URL(
    `${apiBaseV2()}/product/${encodeURIComponent(productId)}/dates`,
  );
  ...
  return requestDeduplicator.deduplicate(requestKey, async () => {
    return monitoredApiCall('TNA-OPTIONS-DATE', async () => {
      ...
      if (!tnaRateLimiter.isAllowed('tna-api')) {
        ...
        throw new Error(
          `API 요청 한도 초과. ${Math.ceil(remainingTime / 1000)}초 후 다시 시도해주세요.`,
        );
      }
        ...
        try {
          const result = await retryWithBackoff(async () => {
            ...
            if (!res.ok) {
              ...
              if (process.env.NODE_ENV === 'development')
                console.log('TNA API 에러 응답:', errorText);
              if (res.status === 503) {
                throw new Error(
                  'TNA API 서버가 일시적으로 사용할 수 없습니다. 잠시 후 다시 시도해주세요.',
                );
              }
            ...
            if (process.env.NODE_ENV === 'development')
              console.log('TNA API 성공 응답:', jsonResult);
            return jsonResult;
          });
...
export async function getProductPriceDateTypeV2(
  productId: string,
  body: unknown,
) {
  ...
  const url = new URL(
    `${apiBaseV2()}/product/${encodeURIComponent(productId)}/price/date-type`,
  );

  if (anyBody?.product_option_code)
    url.searchParams.set(
      'product_option_code',
      String(anyBody.product_option_code),
    );
  if (anyBody?.start_date)
    url.searchParams.set('start_date', String(anyBody.start_date));
  if (anyBody?.end_date)
    url.searchParams.set('end_date', String(anyBody.end_date));
...
export async function getProductPricePeriodTypeV2(
  productId: string,
  body: unknown,
) {
  const anyBody = (body as Record<string, unknown>) || {};
  ...
  const url = new URL(
    `${apiBaseV2()}/product/${encodeURIComponent(productId)}/price/period-type`,
  );
...
export async function getProductDynamicPriceV2(
  productId: string,
  optionId: string,
  body: {
  ...
  if (process.env.NODE_ENV === 'development')
    console.log('getProductDynamicPriceV2 시작:', {
      productId,
      optionId,
      body,
    });
  ...
  if (process.env.NODE_ENV === 'development')
    console.log('TNA API 호출 URL (동적 가격):', url);
  ...
  try {
    const result = await retryWithBackoff(async () => {
      ...
      if (process.env.NODE_ENV === 'development')
        console.log('TNA API 응답 상태 (동적 가격):', res.status);
      if (process.env.NODE_ENV === 'development')
        console.log(
          'TNA API 응답 헤더:',
          Object.fromEntries(res.headers.entries()),
      ...
      if (!res.ok) {
        ...
        const errorText = await res.text().catch(() => '');
        if (process.env.NODE_ENV === 'development')
          console.error('TNA API 에러 응답:', errorText);
        throw new Error(`dynamic-price ${res.status}: ${errorText}`);
      }

      const jsonResult = await res.json();
      if (process.env.NODE_ENV === 'development')
        console.log('TNA API JSON 파싱 성공:', jsonResult);
      return jsonResult;
    });

    if (process.env.NODE_ENV === 'development')
      console.log('TNA API 동적 가격 응답:', result);
    return result;
  } catch (error) {
    if (process.env.NODE_ENV === 'development')
      console.error('TNA API 동적 가격 조회 실패 상세:', {
