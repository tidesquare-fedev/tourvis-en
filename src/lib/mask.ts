...
const repeatStar = (count: number): string =>
  count > 0 ? STAR.repeat(count) : '';
...
export function maskPhone(input: string): string {
  ...
  if (concat.length === 11)
    return `${concat.slice(0, 3)}-${concat.slice(3, 7)}-${concat.slice(7)}`;
  if (concat.length === 10)
    return `${concat.slice(0, 3)}-${concat.slice(3, 6)}-${concat.slice(6)}`;
  // Fallback to generic grouping by 3-4-? if possible
  if (concat.length > 7)
    return `${concat.slice(0, 3)}-${concat.slice(3, concat.length - 4)}-${concat.slice(concat.length - 4)}`;
  return concat;
}
