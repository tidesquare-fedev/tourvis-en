...
class ApiMonitor {
  ...
  recordRequest(
    ...
    current.averageResponseTime =
      (current.averageResponseTime * (current.totalRequests - 1) +
        responseTime) /
      current.totalRequests;
  ...
  private checkAlerts(apiName: string, metrics: ApiMetrics): void {
    ...
    if (errorRate > this.alertThresholds.errorRate) {
      console.warn(
        `🚨 API 알림: ${apiName} 오류율이 ${(errorRate * 100).toFixed(1)}%로 높습니다.`,
      );
    }
    ...
    if (metrics.averageResponseTime > this.alertThresholds.responseTime) {
      console.warn(
        `🚨 API 알림: ${apiName} 평균 응답 시간이 ${metrics.averageResponseTime.toFixed(0)}ms로 느립니다.`,
      );
    }
    ...
    if (metrics.failedRequests >= this.alertThresholds.consecutiveErrors) {
      console.warn(
        `🚨 API 알림: ${apiName}에서 연속 ${metrics.failedRequests}회 오류가 발생했습니다.`,
      );
    }
  }
  ...
  getHealthStatus(apiName: string): 'healthy' | 'degraded' | 'unhealthy' {
    ...
    if (errorRate > 0.3 || metrics.averageResponseTime > 10000)
      return 'degraded';
    return 'healthy';
  }
}
